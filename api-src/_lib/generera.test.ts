...
import { seedForBoard, playSeedForBoard } from './seed'
...
describe('playSeedForBoard — fröet för bottarnas spel', () => {
  test('samma (hemlighet, datum, bricka) ⇒ samma play-frö', () => {
    expect(playSeedForBoard('hemlis', '2026-08-11', 3)).toBe(
      playSeedForBoard('hemlis', '2026-08-11', 3),
    )
  })

  test('play-fröet skiljer sig från giv-fröet för samma bricka', () => {
    // Annars skulle bottarnas slump följa givens delning — vi vill separera dem.
    expect(playSeedForBoard('hemlis', '2026-08-11', 3)).not.toBe(
      seedForBoard('hemlis', '2026-08-11', 3),
    )
  })

  test('byte av hemlighet, datum eller bricka ⇒ annat play-frö', () => {
    const base = playSeedForBoard('hemlis', '2026-08-11', 3)
    expect(playSeedForBoard('annan', '2026-08-11', 3)).not.toBe(base)
    expect(playSeedForBoard('hemlis', '2026-08-12', 3)).not.toBe(base)
    expect(playSeedForBoard('hemlis', '2026-08-11', 4)).not.toBe(base)
  })

  test('play-fröet är ett osignerat 32-bitars heltal', () => {
    const s = playSeedForBoard('hemlis', '2026-08-11', 1)
    expect(Number.isInteger(s)).toBe(true)
    expect(s).toBeGreaterThanOrEqual(0)
    expect(s).toBeLessThanOrEqual(0xffffffff)
  })
