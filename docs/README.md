...
| Fil | 🟢/📘/📦 | Öppna den när … |
|---|---|---|
| **budsystem.md** (379 kB) | 📘 | du ska ändra eller slå upp en **budregel**. Systemboken i paragrafform. **Appens Budsystem-sida renderar den här filen live för ägaren** — varje regeländring MÅSTE skrivas in här. §9 = ändringslogg (visas inte på sidan). |
...
| **imp-tavling-plan.md** | 🟢 | **Dagens IMP (ägarbeslut 2026-09-26, NU — etapp 0–4 BYGGDA i kod på grenen `dagens-imp`, kvar deploy + 0014 + live-prov):** en andra daglig tävling med tolv nya givar räknade i IMP (tabellen, cross-IMP, summan, −3 IMP provisoriskt), de fyra greppen (`form`-kolumnen, frönyckeln, formen som parameter, aggregatet generaliserat), kartan över allt som rörs, etapp 0–5 med grindar och deploy-sekvensen 0013 → deploy → 0014. Öppna den FÖRST vid allt IMP-arbete. |
| **sunt-fornuft-plan.md** | 🟢 | **Sunt förnuft-lagret (NU sedan 2026-09-22):** ägarens oro (pass utan regel), riggen `sunt-fornuft.probe.test.ts`, första mätningen, byggordningen och frågorna till ägaren om hål 1 (advancern över svagt hoppinkliv). |
| **ovanlig-2nt-plan.md** | 📦 | **Ovanlig 2NT — fortsättningen (KLAR & LIVE 2026-09-22; strukturen i budsystem §7.2):** förarbetet + vad motorn gör i dag (provat), bridgebums struktur, frågorna till ägaren, var det byggs (kartan från Michaels-bygget) och lärdomarna från Michaels-dagen. **Läs den först i nästa session.** |
